...
export default function StoryPage() {
  ...
  useEffect(() => {
    async function fetchStory() {
      ...
      if (data) {
        ...
        // Story text still being generated by the edge function — poll until pages arrive
        if (pages.length === 0) {
          setGeneratingText(true);
          if (textPollRef.current) clearInterval(textPollRef.current);
          textPollRef.current = setInterval(async () => {
            ...
            if (fresh?.pages?.length > 0) {
              ...
              // Kick off image generation for the now-available pages
              const missingImages = (fresh.pages as Page[]).filter((p) => p.image_prompt && !p.image_url);
              if (missingImages.length > 0) {
                setLoadingPages(new Set(missingImages.map((p) => p.page_number)));
                ...
                imagePollRef.current = setInterval(async () => {
                  const { data: imgFresh } = await supabase.from('stories').select('pages').eq('id', id).single();
                  if (imgFresh?.pages) {
                    setStory((prev) => prev ? { ...prev, pages: imgFresh.pages } : prev);
                    const stillMissing = imgFresh.pages.filter((p: Page) => p.image_prompt && !p.image_url);
                    setLoadingPages(new Set(stillMissing.map((p: Page) => p.page_number)));
                    if (stillMissing.length === 0) { clearInterval(imagePollRef.current!); imagePollRef.current = null; }
                  }
                }, 5000);
              }
            }
        ...
        if (pages.length > 0 && pagesNeedingImages.length > 0) {
  ...
  if (generatingText) {
    const childName = story?.children?.name || 'your child';
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #FFF8F0 0%, #F5ECD7 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', textAlign: 'center' }}>
        <div style={{ fontSize: '3.5rem', marginBottom: '16px', animation: 'spin 3s linear infinite' }}>✨</div>
        ...
        <h2 className="font-serif" style={{ fontSize: '1.9rem', color: '#4A2C8F', marginBottom: '12px' }}>
          Writing {childName}&apos;s story…
        </h2>
        <p style={{ color: '#888', fontSize: '1rem', maxWidth: '320px', lineHeight: 1.6 }}>
          Claude is crafting a personalised adventure. This usually takes 20–30 seconds.
        </p>
        <div style={{ display: 'flex', gap: '8px', marginTop: '28px' }}>
          {[0,1,2].map(i => (
            <div key={i} style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#4A2C8F', animation: `pulse 1.2s ease-in-out ${i * 0.4}s infinite` }} />
          ))}
        </div>
        <Link href="/dashboard" style={{ marginTop: '36px', color: '#AAA', fontSize: '0.85rem', textDecoration: 'underline' }}>
