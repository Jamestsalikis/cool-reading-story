...
/* the how-it-works and FAQ headings are full sentences, not short beats:
   give them room so they do not stack into a narrow column */
#how-it-works .sec-head,#faq .sec-head{max-width:30ch}
#how-it-works .sec-head h2,#faq .sec-head h2{font-size:var(--s-2)}
@media(min-width:700px){#how-it-works .sec-head,#faq .sec-head{max-width:46ch}}

/* ---- how it works ---- */
.steps{display:grid;grid-template-columns:repeat(3,1fr);gap:clamp(18px,2.4vw,30px);margin-top:clamp(38px,4.5vw,58px);
  max-width:1020px;margin-inline:auto;text-align:left}
@media(max-width:860px){.steps{grid-template-columns:1fr;max-width:520px}}
/* same glass treatment as the name card, so these read as part of the same set */
.step{border-radius:var(--r-lg);padding:clamp(22px,2.4vw,30px);position:relative;
  background:linear-gradient(168deg,rgba(38,44,74,.74),rgba(15,18,36,.82));backdrop-filter:blur(30px) saturate(150%);
  box-shadow:inset 0 1px 0 rgba(255,241,222,.20),inset 0 0 0 1px rgba(255,231,203,.09),
   inset 0 -30px 60px rgba(6,8,20,.28),var(--sh-3)}
.step-n{width:34px;height:34px;border-radius:50%;display:grid;place-items:center;font-weight:800;font-size:.85rem;
  color:#2a1608;background:linear-gradient(180deg,var(--amber-hi),var(--amber-lo));
  box-shadow:0 4px 14px rgba(255,150,60,.32);margin-bottom:16px}
.step h3{font-size:var(--s-1);margin:0 0 .5rem;line-height:1.25;color:var(--txt)}
.step p{color:var(--txt-dim);font-size:var(--s--1);line-height:1.75;margin:0}

/* ---- faq ---- */
.faq{max-width:760px;margin:clamp(38px,4.5vw,58px) auto 0;text-align:left}
.faq-item{border-bottom:1px solid rgba(255,231,203,.12);padding:0 0 1.6rem;margin-bottom:1.6rem}
.faq-item:last-child{border-bottom:0;margin-bottom:0}
.faq-item h3{font-size:var(--s-1);margin:0 0 .55rem;line-height:1.3;color:var(--txt)}
.faq-item p{color:var(--txt-dim);font-size:var(--s--1);line-height:1.8;margin:0}
...
// FAQ copy carried over from the previous homepage, which is what earns the
// "People also ask" placements. Kept verbatim so the wording stays the approved one.
const FAQ = [
  {
  ...
  {
    q: 'Are the illustrations in every personalised story book original?',
    a: 'Yes. Every picture is generated by our own illustration model, trained in-house, so the same child looks like the same child from page one to the last page. Nothing is stock artwork.',
  },
];

// Structured data. Deliberately NO aggregateRating: review markup has to reflect
// real, collected reviews, so it stays out until there is a genuine source for it.
const SCHEMA = {
  '@context': 'https://schema.org',
  '@graph': [
    {
      '@type': 'Organization',
      '@id': 'https://www.talepopstories.com/#organization',
      name: 'TalePop',
      url: 'https://www.talepopstories.com',
      logo: 'https://www.talepopstories.com/brand/talepop-lockup.webp',
      email: 'info@talepopstories.com',
      address: { '@type': 'PostalAddress', addressCountry: 'AU' },
    },
    {
      '@type': 'WebSite',
      '@id': 'https://www.talepopstories.com/#website',
      url: 'https://www.talepopstories.com',
      name: 'TalePop',
      inLanguage: 'en',
      publisher: { '@id': 'https://www.talepopstories.com/#organization' },
    },
    {
      ...
      publisher: { '@id': 'https://www.talepopstories.com/#organization' },
      description:
        'Personalised bedtime stories and illustrated story books for children aged 3 to 10. Every story is written and illustrated around one child, using their name, interests, best friend and pet.',
      audience: { '@type': 'PeopleAudience', suggestedMinAge: 3, suggestedMaxAge: 10 },
      offers: [
        {
          '@type': 'Offer',
          name: 'Monthly',
          price: DISPLAY.aud.monthly,
          priceCurrency: 'AUD',
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* Artwork below the opening screen carries its URL in data-src, so the first
         paint only pays for the hero plate. Each image is promoted to a real src
         when the reader gets near the beat that uses it, with a backstop after load
         so a visitor who never scrolls still ends up with the full page (and so the
         images stay indexable). */
      var deferred=[].slice.call(document.querySelectorAll('img[data-src]'));
      function promoteAt(prog){
        for(var i=deferred.length-1;i>=0;i--){
          var el=deferred[i], at=parseFloat(el.dataset.at||'0');
          if(prog>=at-0.06){
      ...
      (function(){
        function backstop(){ promoteAt(1); }
        if(document.readyState==='complete') setTimeout(backstop,2500);
        else addEventListener('load',function(){ setTimeout(backstop,2500); },{once:true,signal:sig});
      })();
      ...
      var plateFetcher=null;   /* set once the WebGL renderer is up */
      ...
      function tick(now){
        ...
        P_now=P;
        if(deferred.length) promoteAt(P);
        if(plateFetcher) plateFetcher(P);
        paint(P);
      ...
      var PLATES=[
        {el:'bgHero', col:'/journey/hero.avif',  dep:'/journey/hero-depth.avif', from:0.00, to:0.20, fi:0.00, fo:0.055, z0:1.02, z1:1.18, par:1.10, cx:0.0, cy:0.02, shpk:1.15},
        {el:'bgSky', col:'/journey/sky.avif',   dep:'/journey/sky-depth.avif',  from:0.145,to:0.40, fi:0.055,fo:0.06, z0:1.22, z1:1.02, par:0.85, dim:0.60},
        {el:'bgGate', col:'/journey/gate.avif',  dep:'/journey/gate-depth.avif', from:0.345,to:0.70, fi:0.055,fo:0.07, z0:1.02, z1:1.26, par:1.55, dim:0.86},
        {el:'bgBand', col:'/journey/band.avif',  dep:'/journey/band-depth.avif', from:0.78, to:1.01, fi:0.08, fo:0.02, z0:1.02, z1:1.12, par:0.80, dim:1.00, cx:0.0, cy:0.0, shpk:1.15}
      ];
      ...
      (function(){
        ...
        var ready=false;
        /* A plate is only renderable once BOTH its colour and depth textures exist. */
        function isReady(P){ return !!(P.ct && P.dt && P.tw); }

        /* Load one plate. The first plate is fetched immediately so the canvas can go
           live on the opening screen; the rest are fetched after load, which keeps
           ~1.6MB of artwork off the critical path without touching image quality. */
        function loadPlate(P, done){
          P.ct=null; P.dt=null; P.aspect=1.777;
          var left=2;
          function tick(){
            if(--left) return;
            /* drop this plate's CSS stand-in only once the real plate can be drawn */
            if(isReady(P)){ var e=document.getElementById(P.el); if(e) e.style.visibility='hidden'; }
            if(done) done();
          }
          var ci=new Image();
          ci.onload=function(){
          ...
          ci.src=P.col;
          var di=new Image();
          di.onload=function(){ P.dt=upload(di); tick(); };
          di.onerror=tick;
          di.src=P.dep;
        }
        ...
        function loadRest(){
          for(var i=1;i<PLATES.length;i++){
            if(!PLATES[i].queued){ PLATES[i].queued=true; loadPlate(PLATES[i]); }
          }
        }
        /* pull in a plate a little before its beat arrives; the backstop after load
           covers a visitor who never scrolls */
        plateFetcher=function(prog){
          for(var i=1;i<PLATES.length;i++){
            var Q=PLATES[i];
            if(!Q.queued && prog>=Q.from-0.10){ Q.queued=true; loadPlate(Q); }
          }
        };
        loadPlate(PLATES[0], function(){
          if(isReady(PLATES[0])) go();
          else { loadRest(); return; }   /* hero failed: fall back to loading everything */
          if(document.readyState==='complete') setTimeout(loadRest,2500);
          else addEventListener('load',function(){ setTimeout(loadRest,2500); },{once:true,signal:sig});
        });
        ...
        function go(){
          ...
          /* Each plate's CSS stand-in is hidden individually as that plate becomes
             renderable (see loadPlate), so a not-yet-loaded beat still shows art
             instead of black. */
          ...
          (function frame(){
            if(ready){
              ...
              for(var i=0;i<PLATES.length;i++){
                var P=PLATES[i];
                if(!isReady(P)) continue;   /* still downloading: leave it out of the mix */
    ...
    <>
      ...
      <div className="journey" id="journey">
        <div className="stage" id="stage">
          ...
          <img className="bgimg" id="bgHero" src="/journey/hero.avif" alt="" aria-hidden="true" fetchPriority="high" style={{ opacity: 1, objectPosition: '50% 52%' }} />
          <img className="bgimg" id="bgSky" data-at="0.10" data-src="/journey/sky.avif" alt="" aria-hidden="true" />
          <img className="bgimg" id="bgGate" data-at="0.29" data-src="/journey/gate.avif" alt="" aria-hidden="true" />
          <img className="bgimg" id="bgBand" data-at="0.72" data-src="/journey/band.avif" alt="" aria-hidden="true" style={{ objectPosition: '50% 50%' }} />
          <canvas id="depth-canvas" aria-hidden="true"></canvas>

          <div className="portals" id="portals" aria-hidden="true">
            <div className="portal"><img data-src="/journey/world-space.avif" data-at="0.12" alt="A young astronaut exploring a glowing planet in a personalised space bedtime story" decoding="async" /><span>Beyond the last planet</span></div>
            <div className="portal"><img data-src="/journey/world-unicorn.avif" data-at="0.12" alt="A child riding a winged unicorn through pink clouds in a personalised bedtime story" decoding="async" /><span>The kingdom in the clouds</span></div>
            <div className="portal"><img data-src="/journey/world-dino.avif" data-at="0.12" alt="A child meeting a friendly dinosaur in a personalised dinosaur bedtime story" decoding="async" /><span>The gentle giant</span></div>
            <div className="portal"><img data-src="/journey/world-pirate.avif" data-at="0.12" alt="A child as a pirate captain at sea in a personalised adventure bedtime story" decoding="async" /><span>Captain for one night</span></div>
            <div className="portal"><img data-src="/journey/world-ocean.avif" data-at="0.12" alt="A child and a whale underwater in a personalised ocean bedtime story" decoding="async" /><span>The whale who waited</span></div>
            <div className="portal"><img data-src="/journey/world-hero.avif" data-at="0.12" alt="A child in a superhero cape in a personalised superhero bedtime story" decoding="async" /><span>The cape stays on</span></div>
          </div>
          ...
          <div className="beats" id="beats">
            ...
            <section className="beat" data-beat="0">
              <div className="wrap">
                ...
                <h1>Tonight's <em>personalised bedtime story</em> has their name in it.</h1>
                <p className="lead">Not a name dropped into a stock story. A brand new personalised story book for kids, written and illustrated around one child, ready before lights out. Made for children aged 3 to 10.</p>
                <div className="acts">
            ...
            <section className="beat" data-beat="1">
              <div className="wrap">
                ...
                <h2>So. Who is the story for?</h2>
                <p className="lead">Type your child's name and watch the personalised story change around them.</p>
            ...
            <section className="beat mid" data-beat="2">
              <div className="wrap">
                ...
                <p className="lead">Your child's interests pick the world. Dinosaurs this month, deep sea the next. Every personalised bedtime story gets a different plot.</p>
              </div>
            </section>
            ...
            <section className="beat" data-beat="4">
              <div className="wrap spreadwrap">
                <div className="spread">
                  ...
                  <div className="spread-img"><img data-src="/journey/spread.avif" data-at="0.45" alt="A page from a personalised bedtime story book: a child and a small green dragon in front of a glowing castle" decoding="async" /></div>
                </div>
              </div>
            </section>
      ...
      <section className="sec" id="pricing">
        <div className="wrap">
          <div className="sec-head">
            ...
            <p>Start with one free personalised story book. Cancel in a single tap, from inside the app.</p>
          </div>
      ...
      <section className="sec" id="how-it-works">
        <div className="wrap">
          <div className="sec-head">
            ...
            <h2>Get a personalised bedtime story in three steps</h2>
            <p>Two minutes to set up. Their first personalised story book is ready to read at bedtime tonight.</p>
          </div>
          <div className="steps">
            <div className="step">
              <div className="step-n">1</div>
              <h3>Tell us about your child</h3>
              <p>Their name, age, what makes them laugh, their best friend, their pet. The more you share, the richer their personalised story.</p>
            </div>
            <div className="step">
              <div className="step-n">2</div>
              <h3>We write their personalised story from scratch</h3>
              <p>A unique illustrated adventure invented entirely around your child. No two TalePop personalised story books have ever been the same.</p>
            </div>
            <div className="step">
              <div className="step-n">3</div>
              <h3>Read their personalised bedtime story tonight</h3>
              <p>Open it up, read it aloud, and watch their face when they realise the adventure is theirs.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="sec" id="faq">
        <div className="wrap">
          <div className="sec-head">
            ...
            <h2>Questions parents ask about our personalised bedtime stories</h2>
          </div>
          <div className="faq">
            {FAQ.map((f) => (
      ...
      <section className="sec" id="safety" style={{ paddingTop: 'clamp(20px,3vw,44px)' }}>
        <div className="wrap">
          ...
          <div className="sec-head">
            ...
            <h2>Their first personalised story book is free. It can be ready before bedtime.</h2>
            <p>No ads, no third party tracking, no data sold. Reading level matched to their age, and nothing in a TalePop personalised bedtime story you would not read aloud yourself. Delete the profile and it is gone.</p>
          </div>
